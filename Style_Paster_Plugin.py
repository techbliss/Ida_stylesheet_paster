...
from idaapi import *
import sys
sys.path.insert(0 , idaapi.idadir("plugins\\Style_Paster")) #change path to the icon
import ico
from ico import *
...
class MyEditorHandlerPaste(idaapi.action_handler_t): # the class name must be unique for each plugin nb same as line (53)
    def __init__(self):
    ...
    def activate(self, ctx):
        g = globals()
        idahome = idaapi.idadir("plugins\\Style_Paster") #change to set plguin path where the main plguin script is
        IDAPython_ExecScript(idahome + "\\Style_Paste_main.py", g) #change for the main plugin script

    def update(self, ctx):
        return idaapi.AST_ENABLE_ALWAYS

class ripeyePaste(idaapi.plugin_t): #change class to unique name for each plugin , also change line (103)
    ...
    def editor_menuactionPaste(self): #change for something unique.also change line (90)
        action_desc = idaapi.action_desc_t(
            ...
            idaapi.load_custom_icon(idaapi.idadir("plugins\\Style_Paster")+"\\iconss.png")  # hackish load action icon , if no custom icon use number from 1-150 from internal ida
        )
        ...
        idaapi.attach_action_to_popup(form, None, "my:editoractionPaste", None) # The action ID (see line(51)
    ...
    def run(self, arg = 0):
        ...
        idaapi.msg("StyleSheet Paster Loaded to menu \n use Alt+P hot key to quick load ")
